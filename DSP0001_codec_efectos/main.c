...
float amplitud2 = 0.25;
float amplitud3 = 0.75;
float amplitud4 = 0.15;                         //Parámetro para variar al ampilitud del eco
...
//c6713dskinit.h
/*#####################-C6713dskinit.h-########################*/
#define LEFT  0                  //data structure for union of 32-bit data
#define RIGHT 1                  //into two 16-bit data
union {
    Uint32 uint;
    short channel[2];
    } AIC_data;
...
//1. Bucle transparente
 ...
 {
 salida=entrada + amplitud*buffer[i] + amplitud2*buffer2[j];                           //Muestra reciente+muestra anterior
output_sample(salida);
 }
else if(DSK6713_DIP_get(3) == 0)
 {
 salida=entrada + amplitud*buffer[i] + amplitud2*buffer2[j] + amplitud3*buffer3[K] + amplitud4*buffer4[L];                           //Muestra reciente+muestra anterior
output_sample(salida);
 }
 ...
 buffer3[K]=entrada;
 buffer4[L]=entrada;
 i++; j++;   K++; L++;                                                                      //Incremento de la cuenta en el Buffer
 if (i >= lb1) i = 0;
if (j >= lb2) j = 0;
if (K >= lb3) K = 0;
if (L >= lb4) L = 0;
